  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ChangeNotifierProvider(
        create: (_) => LcMasterViewmodel(context: context, entry: "LcMasterSsListScreen", ssCode: "-1"),
        child: Consumer<LcMasterViewmodel>(
          builder: (context, viewModel, child) {
            return viewModel.isLoading
                ? const Center(child: CircularProgressIndicator())
                : viewModel.lcMasterSsList.isEmpty
                ? const Center(child: Text("No SS Section founded."),)
                : ListView.separated(
                    ...
                    itemBuilder: (_, int index) => ListTile(
                      ...
                      onTap: () {
                        var argument = {
                          'ssCode': viewModel.lcMasterSsList[index].optionId!,
                          'ssName': viewModel.lcMasterSsList[index].optionName!
                        };
