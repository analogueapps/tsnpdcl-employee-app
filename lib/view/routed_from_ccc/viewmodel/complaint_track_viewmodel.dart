...
class ComplaintTackViewModel extends ChangeNotifier{
  ComplaintTackViewModel({required this.context, required this.ticketId}) {
    getTicketTrack(ticketId);
  }
  ...
  final List<CCCComplaintTrack> _complaintTrack = [];
  List<CCCComplaintTrack> get complaintTrack => _complaintTrack;


  Future<bool> getTicketTrack(String ticketId ) async {
    _isLoading = isTrue;
    notifyListeners();
          ...
          if (response.data['sessionValid'] == true) {
            if (response.data['taskSuccess'] == true) {
              ...
              if (dataListRaw != null && dataListRaw.isNotEmpty) {
                ...
                _complaintTrack.addAll(dataList);
