  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ChangeNotifierProvider(
        create: (_) => LcMasterViewmodel(context: context, entry: "FeederInductionListScreen", ssCode: args['fdrCode']),
        child: Consumer<LcMasterViewmodel>(
          builder: (context, viewModel, child) {
            return viewModel.isLoading
                ? const Center(child: CircularProgressIndicator())
                : viewModel.inductionPointsOfFeederList.isEmpty
                ? const Center(child: Text("No induction points feeder list founded."),)
                : ListView.builder(
                  itemCount: viewModel.inductionPointsOfFeederList.length,
                  itemBuilder: (context, index) {
                    final item = viewModel.inductionPointsOfFeederList[index];
                                ...
                                children: [
                                  ...
                                  Text(
                                    ...
                                    style: const TextStyle(
                                      ...
                                      fontSize: 10, // Specify a font size for better consistency
                                    ),
                                  ),
                                ],
                              ),
