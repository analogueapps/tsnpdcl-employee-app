...
class MaintenanceDueViewModel extends ChangeNotifier {
  ...
  Future<void> getPendingMaintenanceList() async {
    ...
    var response = await ApiProvider(baseUrl: Apis.SS_END_POINT_BASE_URL)
        .postApiCall(context, Apis.GET_SS_MAINTENANCE, payload);
    _isLoading = isFalse;
          ...
          if (response.data['sessionValid'] == isTrue) {
            if (response.data['taskSuccess'] == isTrue) {
              if (response.data['dataList'] != null) {
                ...
                final List<MaintenanceFinishedModel> dataList = jsonList
                    .map((json) => MaintenanceFinishedModel.fromJson(json))
                    .toList();
