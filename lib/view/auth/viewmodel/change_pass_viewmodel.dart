...
class ChangePassViewmodel extends ChangeNotifier {
  ...
  final TextEditingController empConPassController = TextEditingController();
  ...
  ChangePassViewmodel({required this.context, required this.empId}) {
    _initPackageInfo();
          ...
          if (response.statusCode == successResponseCode) {
            if (response.data['success'] == isTrue) {
              if(response.data['objectJson'] != null) {
                ...
                var argument = {
                  'phone': jsonList[0],
                  'pass': empPassController.text.trim(),
                  'mode': 2,
                };
