...
List<DetailTicketModel> get complaintDetailsList => _complaintDetailsList;
  ...
  Future<bool> getDetailedTicket(String ticketId ) async {
            ...
            if (response.data['taskSuccess'] == isTrue) {
              ...
              if (dataList is List && dataList.isEmpty) {
                ...
                if (complaintData.isNotEmpty) {
                  final complaintModel = DetailTicketModel.fromJson(complaintData);
                  _complaintDetailsList.add(complaintModel);

                  print("Fetched complaint: ${_complaintDetailsList.length}"); // Or another field
                }
              }
            }
          } else {
             ...
             builder: (context, setState)
             {
               return AlertDialog(
                   ...
                   actions: [
                     ...
                     TextButton(
                       onPressed: () {
                         callConsumer( complaintDetailsList[0].ticketNumber!, complaintDetailsList[0].mobileNo??"", registeredNumber.text );
                       },
                       style: ButtonStyle(
   ...
   void resetDialogValues() {
     registeredNumber.text=complaintDetailsList[0].registeredMobileNumber??"";
     notifyListeners();
   }
}
