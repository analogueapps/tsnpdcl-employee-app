...
class CategoryChangeDetailViewmodel extends ChangeNotifier {
  ...
  List<EroModel> get reasonList =>
      makeCapacity.isNotEmpty && makeCapacity[0].rejectReasons.isNotEmpty
          ? makeCapacity[0].rejectReasons //${reason.optionId} (${reason.optionName})'
          .toList()
          : [];
  ...
  List<EroModel> get capacityList =>
      makeCapacity.isNotEmpty && makeCapacity[0].meterCapacityList.isNotEmpty
          ? makeCapacity[0].meterCapacityList
          .toList()
          : [];
  ...
  Future<void> assignDtrMaintenance(num regID, String? selectedEmpId) async {
    ...
    var response = await ApiProvider(baseUrl: Apis.CAT_CHANGE_ENDPOINT_URL).postApiCall(context, Apis.UPDATE_CAT_CHANGE, payload);
    if (context.mounted) {
      ProcessDialogHelper.closeDialog(context);
    }
 ...
 final  List<MeterMakeAndCapacityModel> makeCapacity=[];
  Future<void> getMeterMakesAndCapacities() async {
    ...
    var response = await ApiProvider(baseUrl: Apis.CAT_CHANGE_ENDPOINT_URL).postApiCall(context, Apis.MAKE_CAPACITIES, payload);
    if (context.mounted) {
      ProcessDialogHelper.closeDialog(context);
    }
        ...
        if (response.statusCode == successResponseCode) {
          if (response.data['taskSuccess'] == isTrue) {
            if(response.data['dataList'] != null) {
              ...
              final List<MeterMakeAndCapacityModel> dataList = jsonList.map((json) => MeterMakeAndCapacityModel.fromJson(json)).toList();
  ...
  Future<void> updateCatChangeApplication(num regID, String? selectedEmpId, String imageUrl) async {

    String? empType=SharedPreferenceHelper.getStringValue(LoginSdkPrefs.empType);
