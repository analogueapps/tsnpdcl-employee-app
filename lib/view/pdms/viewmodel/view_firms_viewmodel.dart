...
class ViewFirmsViewmodel extends ChangeNotifier {
  ...
  final List<PoleManufacturingFirmEntity> _poleManufacturingFirmList = [];
  List<PoleManufacturingFirmEntity> get poleManufacturingFirmList => _poleManufacturingFirmList;

  // Constructor to initialize the items
  ViewFirmsViewmodel({required this.context}) {
    _loadUser();
    getFirms();
  }
  ...
  Future<void> getFirms() async {
    _poleManufacturingFirmList.clear();
    _isLoading = isTrue;
    notifyListeners();
    ...
    var response = await ApiProvider(baseUrl: Apis.PDMS_END_POINT_BASE_URL).postApiCall(context, Apis.GET_POLE_FIRMS_URL, payload);
    _isLoading = isFalse;
        ...
        if (response.statusCode == successResponseCode) {
          ...
          if (response.data['taskSuccess'] == isTrue) {
            if(response.data['dataList'] != null) {
              ...
              final List<PoleManufacturingFirmEntity> dataList = jsonList.map((json) => PoleManufacturingFirmEntity.fromJson(json)).toList();
              _poleManufacturingFirmList.addAll(dataList);
              notifyListeners();
            }
          }
          // } else {
  ...
  void filterFabClicked(String data) {
    Navigation.instance.navigateTo(Routes.createFirmScreen,args: data, onReturn: (result) {
      if(result != null) {
        getFirms();
      }
    });
